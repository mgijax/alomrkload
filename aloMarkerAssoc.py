...
#	required: DBSERVER, DBNAME, DBUSER, and (DBPASSWORD or DBPASSWORDFILE)
#	optional: LOGDIR, LOGDEBUG
...
def processCommandLine():
	...
	if environ.has_key('LOGDEBUG'):
		if environ['LOGDEBUG'].lower() != 'false':
			VERBOSE = True

	if environ.has_key('DBPASSWORD'):
		password = environ['DBPASSWORD']
	elif environ.has_key('DBPASSWORDFILE'):
		try:
			fp = open (environ['DBPASSWORDFILE'], 'r')
			password = fp.readline().strip()
			fp.close()
		except:
			bailout ('Cannot read password from: %s' % value)
	else:
		bailout('Must specify either DBPASSWORD or DBPASSWORDFILE' + \
